# for debug
output "docker_proxies" {
  value = local.docker_proxies
}
...
resource "nexus_repository_docker_proxy" "docker-proxies" {
  for_each = {
    for proxy in local.docker_proxies : "${proxy.name}-${proxy.type}-proxy" => proxy
  }
  ...
  docker_proxy {
    index_type = each.value.docker_proxy.index_type
    index_url  = each.value.docker_proxy.index_url
  }
